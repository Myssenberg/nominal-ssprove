...
Definition QSET_loc (N : NIKE_scheme) : Location := (chMap 'SID N 'nat ; 80).

Definition KEY_locs_tt (N: NIKE_scheme) := fset [:: SID_loc N ; K_loc N ; QSET_loc N].  
Definition KEY_locs_ff (N: NIKE_scheme) := fset [:: SID_loc N ; K_loc N ; QSET_loc N].
...
Definition KEY (N: NIKE_scheme) qset (b : 'bool) :
  ...
  [module KEY_locs_tt N;
    #def #[ SET ] ('(sid, k) : 'SID N × 'shared_key N): ('unit) {
      ...
      #assert isSome (QSETLOC sid) as count ;;
      let counts := getSome (QSETLOC sid) count in
      #assert (counts < qset)%N ;;
      #put (QSET_loc N) := setm QSETLOC sid (counts.+1) ;;

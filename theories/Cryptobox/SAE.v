...
Definition I_SAE_OUT (n m c: choice_type) :=
  [interface
    #val #[ GEN ]: 'unit → 'unit ;    
    #val #[ ENC ]: ('T m × 'T n) → 'unit (*'T c*)  ;
    #val #[ DEC ]: ('T c × 'T n) → 'unit (*'T m*) 
].

Definition SAE (b : bool) (m c : choice_type) (n k : finType) `{Positive #|n|} `{Positive #|k|} (sample_K : code fset0 [interface] 'fin #|k|) (sample_C : code fset0 [interface] c) (enc : forall (m : m) (k : k) (n : n), code fset0 [interface] c) (dec : forall (c : c) (k : k) (n : n), code fset0 [interface] m):
